      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div data-noprint className="flex flex-wrap items-center gap-2">
          ...
          <Button
            ...
            onClick={() => {
              csvIndir(`rapor-${id ?? "rapor"}`, [t("Alan"), t("Değer")], [
                [t("Rapor Adı"), t(ad)],
                [t("Kategori"), kategori ? t(KATEGORI_META[kategori].etiket) : "-"],
                [t("Format"), rapor?.format ?? "-"],
                [t("Sıklık"), rapor?.siklik ? t(rapor.siklik) : "-"],
                [t("Son Oluşturulma"), rapor?.sonOlusturma ?? "-"],
              ]);
              toast.success(`${t(ad)} ${t("Excel'e aktarıldı")}`);
            }}
          >
          ...
          <Button
            ...
            onClick={() => {
              toast(t("PDF için 'Hedef' olarak 'PDF olarak kaydet' seçin"));
              window.print();
            }}
          >
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="py-16 text-center">
            <p className="text-sm text-muted-foreground">{t("Rapor bulunamadı.")} <Link href="/raporlar" className="font-medium text-primary">{t("Rapor listesine dön")}</Link></p>
          </CardContent>
        </Card>
      )}
    </div>
