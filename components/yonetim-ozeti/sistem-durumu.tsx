  ...
  return (
    <Card className="h-full">
      <CardHeader className="flex-row items-center justify-between gap-4 space-y-0">
        <h3 className="font-heading text-base font-medium">{t("Sistem Durumu")}</h3>
          ...
          <ul className="divide-y">
            {data.sistem.map((s) => (
              <li key={s.alan} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0">
                ...
                />
                <span className="flex-1 text-sm">{t(s.alan)}</span>
                <span className="text-sm font-medium text-muted-foreground">{t(s.durum)}</span>
              </li>
            ))}
          </ul>
        )}
