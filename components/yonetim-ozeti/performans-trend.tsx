  ...
  return (
    <Card className="h-full">
      <CardHeader className="flex-row items-center justify-between gap-4 space-y-0">
        <h3 className="font-heading text-base font-medium">{t("Enerji Performans Trendi")}</h3>
        <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
          <span className="inline-flex items-center gap-1.5"><span className="h-0.5 w-4 rounded-full bg-teal-500" /> {t("Gerçekleşen")}</span>
          <span className="inline-flex items-center gap-1.5"><span className="h-0.5 w-4 rounded-full border-t-2 border-dashed border-emerald-500" /> {t("Hedef")}</span>
          <span className="inline-flex items-center gap-1.5"><span className="h-0.5 w-4 rounded-full border-t-2 border-dashed border-slate-400" /> {t("Baz Yıl (2024)")}</span>
        </div>
      </CardHeader>
      <CardContent>
        {isLoading || !data ? (
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={data.performansTrend} margin={{ left: 8, right: 8, top: 12, bottom: 4 }}>
                ...
                <Tooltip
                  ...
                  formatter={(v, n) => [iki(Number(v)), n === "gerceklesen" ? t("Gerçekleşen") : n === "hedef" ? t("Hedef") : t("Baz Yıl")]}
                />
