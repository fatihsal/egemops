  ...
  return (
    <Card className="h-full">
      <CardHeader className="flex-row items-center justify-between gap-4 space-y-0">
        <h3 className="font-heading text-base font-medium">{t("Aylık Enerji Tüketimi ve Üretim")}</h3>
        <div className="flex items-center gap-4 text-xs text-muted-foreground">
          <span className="inline-flex items-center gap-1.5"><span className="size-2.5 rounded-full" style={{ background: TUKETIM }} /> {t("Enerji Tüketimi (TEP)")}</span>
          <span className="inline-flex items-center gap-1.5"><span className="h-0.5 w-4 rounded-full" style={{ background: URETIM }} /> {t("Üretim (ton)")}</span>
        </div>
      </CardHeader>
      <CardContent>
        {isLoading || !data ? (
            ...
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={data.tuketimUretim} margin={{ left: 4, right: 4, top: 12, bottom: 4 }}>
                ...
                <Tooltip
                  ...
                  formatter={(v, n) => [`${sayi(Number(v))} ${n === "tuketim" ? "TEP" : "ton"}`, n === "tuketim" ? t("Enerji Tüketimi") : t("Üretim")]}
                />
