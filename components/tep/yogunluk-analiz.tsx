...
export function TepYogunlukAnaliz() {
  ...
  const kartlar = [
    { etiket: t("Güncel Yoğunluk"), deger: yogKpi?.deger ?? uc(aylar[6].yogunluk), birim: "TEP/ton", ikon: "solar:speedometer-max-bold-duotone", sinif: "bg-teal-50 text-teal-600 dark:bg-teal-950 dark:text-teal-300", alt: "Temmuz 2026" },
    { etiket: t("Ortalama Yoğunluk"), deger: uc(ort), birim: "TEP/ton", ikon: "solar:chart-2-bold-duotone", sinif: "bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300", alt: t("YTD ortalama") },
    { etiket: `${t("Baz Yıl")} (${bz.bazYil})`, deger: bz.bazDeger, birim: "TEP/ton", ikon: "solar:flag-bold-duotone", sinif: "bg-violet-50 text-violet-600 dark:bg-violet-950 dark:text-violet-300", alt: t("Referans değer") },
    { etiket: t("Baz Yıla Göre İyileşme"), deger: `%${sayiOndalik(Math.abs(bz.iyilesme))}`, birim: "", ikon: "solar:graph-down-bold-duotone", sinif: "bg-emerald-50 text-emerald-600 dark:bg-emerald-950 dark:text-emerald-300", alt: t("Daha az = daha iyi"), vurgu: true },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 items-stretch gap-6 xl:grid-cols-12">
        <Card className="h-full xl:col-span-8">
          <CardHeader>
            <h3 className="font-heading text-base font-medium">{t("Aylık Enerji Yoğunluğu")} <span className="text-sm font-normal text-muted-foreground">(TEP / ton)</span></h3>
          </CardHeader>
          <CardContent>
        ...
        <Card className="h-full xl:col-span-4">
          <CardHeader>
            <h3 className="font-heading text-base font-medium">{t("Yıllık Yoğunluk")} <span className="text-sm font-normal text-muted-foreground">(TEP / ton)</span></h3>
          </CardHeader>
          <CardContent>
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-wrap items-center justify-between gap-3">
            <h3 className="font-heading text-base font-medium">{t("Üretim ve Enerji Yoğunluğu")}</h3>
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs text-muted-foreground">
              <span className="inline-flex items-center gap-1.5"><span className="size-2 rounded-full bg-slate-400" /> {t("Üretim (ton)")}</span>
              <span className="inline-flex items-center gap-1.5"><span className="h-[3px] w-3.5 rounded-full bg-teal-600" /> {t("Yoğunluk (TEP/ton)")}</span>
            </div>
          </div>
        </CardHeader>
        <CardContent>
