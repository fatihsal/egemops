...
import type { AkaryakitFirsat } from "@/lib/types";

const DURUM_SINIF: Record<AkaryakitFirsat["durum"], string> = {
...
export function AkaryakitVerimlilikFirsatlari() {
  ...
  return (
    <Card className="h-full">
      <CardHeader>
        <h3 className="font-heading text-base font-medium">{t("Verimlilik Fırsatları")}</h3>
      </CardHeader>
            ...
            <ul className="divide-y">
              {data.firsatlar.map((f) => (
                <li key={f.baslik}>
                  ...
                  >
                    <span className="flex size-9 shrink-0 items-center justify-center rounded-lg bg-amber-50 text-amber-600 dark:bg-amber-950 dark:text-amber-300">
                      <Icon icon="solar:lightbulb-bolt-bold-duotone" className="size-5" />
                    </span>
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium">{t(f.baslik)}</p>
                      <p className="truncate text-xs text-muted-foreground">{t(f.aciklama)}</p>
                    </div>
