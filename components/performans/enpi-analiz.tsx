    ...
    <div className="space-y-6">
              ...
              <CardContent className="space-y-3">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className={cn("rounded-full px-2 py-0.5 text-[11px] font-medium", asildi ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300" : "bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-300")}>
                    {asildi ? "Hedef Aşıldı" : "Takipte"}
                  </span>
                </div>
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div><p className="text-[11px] text-muted-foreground">Baz</p><p className="mt-0.5 font-heading font-bold tabular-nums text-muted-foreground">{h.baz}</p></div>
                  <div><p className="text-[11px] text-muted-foreground">Hedef</p><p className="mt-0.5 font-heading font-bold tabular-nums">{h.hedef}</p></div>
                  <div><p className="text-[11px] text-muted-foreground">Gerçek</p><p className="mt-0.5 font-heading font-bold tabular-nums text-teal-600 dark:text-teal-400">{h.gercek}</p></div>
                </div>
                <div className="space-y-1">
                  ...
                  <p className="text-[11px] text-muted-foreground">Hedefe ilerleme: %{Math.min(100, Math.max(0, ilerleme))}</p>
                </div>
              </CardContent>
            </Card>
          );
      ...
      <div className="grid grid-cols-1 items-stretch gap-6 xl:grid-cols-12">
        <Card className="h-full xl:col-span-8">
          <CardHeader>
            <div className="flex flex-wrap items-center justify-between gap-3">
              <h3 className="font-heading text-base font-medium">Aylık EnPI: Hedef vs Gerçek <span className="text-sm font-normal text-muted-foreground">(TEP/ton)</span></h3>
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs text-muted-foreground">
                <span className="inline-flex items-center gap-1.5"><span className="h-0 w-4 border-t-2 border-dashed border-green-500" /> Hedef</span>
                <span className="inline-flex items-center gap-1.5"><span className="h-[3px] w-3.5 rounded-full bg-teal-600" /> Gerçekleşen</span>
              </div>
            </div>
          </CardHeader>
          <CardContent>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={data.aylik} margin={{ left: 4, right: 8, top: 8, bottom: 0 }}>
                  ...
                  <Tooltip
                    ...
                    formatter={(value, name) => [`${uc(Number(value))} TEP/ton`, name === "hedefEnPI" ? "Hedef" : "Gerçekleşen"]}
                  />
        ...
        <Card className="h-full xl:col-span-4">
          <CardHeader>
            <h3 className="font-heading text-base font-medium">Hedef İlerlemesi</h3>
          </CardHeader>
          <CardContent className="flex-1">
            <div className="flex h-full flex-col items-center justify-center gap-4">
              <RadialOran deger={data.radial.oran} renk="#0d9488" className="size-40" yaziSinif="text-3xl" />
              <p className="text-center text-xs text-muted-foreground">2026 hedefinin <span className="font-semibold text-foreground">%{data.radial.oran}</span>'ü gerçekleşti</p>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          <h3 className="font-heading text-base font-medium">Aylık Sapma <span className="text-sm font-normal text-muted-foreground">(Gerçek − Hedef, TEP/ton)</span></h3>
        </CardHeader>
        <CardContent>
