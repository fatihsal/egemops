...
export function KullaniciRoller() {
  ...
  const toplam = data?.roller.reduce((t, r) => t + r.adet, 0) ?? 0;

  return (
    <Card className="h-full">
      <CardHeader>
        <h3 className="font-heading text-base font-medium">Roller & İzinler</h3>
      </CardHeader>
            ...
            return (
              <div key={r.rol} className="rounded-xl border p-3">
                <div className="flex items-center justify-between gap-2">
                  <span className={cn("inline-block rounded-full px-2.5 py-0.5 text-[11px] font-medium", m.sinif)}>{m.etiket}</span>
                  <span className="text-sm font-semibold tabular-nums">{r.adet} <span className="text-xs font-normal text-muted-foreground">kullanıcı</span></span>
                </div>
