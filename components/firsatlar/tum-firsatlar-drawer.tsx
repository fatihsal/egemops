    ...
    <Sheet>
      ...
      <SheetContent side="right" className="flex w-full flex-col gap-0 p-0 sm:max-w-xl">
        <SheetHeader className="border-b p-5">
          <SheetTitle>{t("Tüm Fırsatlar")}</SheetTitle>
          <SheetDescription>
            {sirali.length} {t("fırsat")} · {t("toplam")} {sayiOndalik(toplamTasarruf)} {t("TEP/yıl potansiyel tasarruf")}
          </SheetDescription>
        </SheetHeader>
              ...
              <div key={f.id} className="flex items-center gap-3 rounded-xl border p-3">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={cn("shrink-0 rounded-full px-2 py-0.5 text-[10px] font-medium", o.sinif)}>{t(o.etiket)}</span>
                  </div>
                  <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs text-muted-foreground">
                    ...
                    <span className="tabular-nums">{t("Yatırım")} {sayi(f.yatirim)} €</span>
                    <span className="tabular-nums">{sayiOndalik(f.geriDonus)} {t("yıl")}</span>
                  </div>
                </div>
                <div className="flex shrink-0 flex-col items-end gap-1.5">
                  <span className={cn("rounded-full px-2 py-0.5 text-[10px] font-medium whitespace-nowrap", d.sinif)}>{t(d.etiket)}</span>
