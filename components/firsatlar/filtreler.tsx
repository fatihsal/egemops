  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <Select value={kaynak} onValueChange={(v) => setKaynak(v as string)}>
        ...
        <SelectContent>{KAYNAKLAR.map((k) => <SelectItem key={k} value={k}>{k}</SelectItem>)}</SelectContent>
      </Select>
      ...
      <Button className="h-9 gap-1.5 bg-teal-600 text-white shadow-sm hover:bg-teal-700" onClick={() => toast.success("Yeni fırsat formu açılıyor")}>
