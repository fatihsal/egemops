  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <div className="flex items-center gap-2 rounded-lg border bg-card pl-3 shadow-sm">
        <span className="text-xs font-medium text-muted-foreground">Katsayı Yılı</span>
      ...
      <Button variant="outline" className="h-9 gap-1.5 bg-card" onClick={() => toast("Resmi katsayı setinden içe aktarılıyor")}>
        <Icon icon="solar:import-bold-duotone" className="size-4.5 text-muted-foreground" />
        Resmi Setten Al
      </Button>
      <Button className="h-9 gap-1.5 bg-teal-600 text-white shadow-sm hover:bg-teal-700" onClick={() => toast.success("Katsayı değişiklikleri kaydedildi")}>
