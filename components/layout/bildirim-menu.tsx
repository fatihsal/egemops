      ...
      >
        ...
        <div className="flex items-center justify-between border-b px-3.5 py-2.5">
          <div className="flex items-center gap-2">
            <span className="text-sm font-semibold">{t("Bildirimler")}</span>
            {okunmamis > 0 ? (
        ...
        <div className="max-h-80 overflow-y-auto [scrollbar-width:thin] [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-muted-foreground/30 [&::-webkit-scrollbar]:w-1.5">
          {bildirimler.length === 0 ? (
            <div className="flex flex-col items-center gap-1.5 py-10 text-center">
              ...
              <p className="text-xs text-muted-foreground">{t("Bildiriminiz yok")}</p>
            </div>
          ) : (
            bildirimler.map((b) => (
              <button
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 line-clamp-2 text-xs text-muted-foreground">{t(b.aciklama)}</p>
                  <p className="mt-1 text-[11px] text-muted-foreground/80">{t(b.zaman)}</p>
                </div>
              </button>
            ))
          )}
        ...
        <div className="border-t p-1.5">
          <button
            type="button"
            onClick={() => toast(t("Tüm bildirimler yakında"))}
